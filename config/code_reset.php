...
function send_reset_code($name,$email,$token){
  ...
  $mail->addAddress($email);     //Add a recipient
    ...
    <a href='http://localhost/noveltees/passport_change.php?token=$token&email=$email'>Click me</a>
    ";
...
if(isset($_POST['submit'])){
    ...
    $token=md5(rand());
    $sql= "SELECT email,name FROM  customer where email='$email'";
  ...
  if( mysqli_num_rows($check) > 0){
    ...
    $_SESSION['status']="A reset link has sent in your mail.Thanks for stay with us";
